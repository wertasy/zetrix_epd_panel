...
typedef struct _lv_font_t {
    bool (*get_glyph_dsc)(const struct _lv_font_t *font, lv_font_glyph_dsc_t *dsc_out, uint32_t letter,
...
static inline bool lv_font_get_glyph_dsc(const lv_font_t *font, lv_font_glyph_dsc_t *dsc_out, uint32_t letter,
                                         uint32_t letter_next)
{
...
static inline const void *lv_font_get_glyph_bitmap(lv_font_glyph_dsc_t *g_dsc, struct _lv_draw_buf_t *draw_buf)
{
    if (!g_dsc || !g_dsc->resolved_font || !g_dsc->resolved_font->get_glyph_bitmap)
        return NULL;
