...
esp_err_t nfc_write_uri_ndef(const char *uri)
{
    ...
    if (suffix_len > NFC_USER_DATA_CAPACITY || (13 + suffix_len) > NFC_USER_DATA_CAPACITY) {
        return ESP_ERR_INVALID_ARG;
    }

    uint8_t *ndef_msg = malloc(8 + suffix_len);
    if (!ndef_msg) {
...
{
    if (!out_message || !out_len)
        return ESP_ERR_INVALID_ARG;

    uint8_t *raw = malloc(NFC_USER_DATA_CAPACITY);
    if (!raw) {
        return ESP_ERR_NO_MEM;
    }

    esp_err_t ret = nfc_read_user_data(0, raw, NFC_USER_DATA_CAPACITY);
    if (ret != ESP_OK) {
        free(raw);
        return ret;
    }
    ...
    size_t cursor = 0;
    while (cursor < NFC_USER_DATA_CAPACITY) {
        ...
        if (cursor >= NFC_USER_DATA_CAPACITY) {
            free(raw);
        ...
        if (length == 0xFF) {
            if (cursor + 1 >= NFC_USER_DATA_CAPACITY) {
                free(raw);
                return ESP_ERR_INVALID_RESPONSE;
            }
        ...
        if (cursor + length > NFC_USER_DATA_CAPACITY) {
            free(raw);
            return ESP_ERR_INVALID_RESPONSE;
        }
...
/**
 * Write a pre-formatted NDEF TLV blob (tag 0x03 ... terminator 0xFE) directly
 * to the tag user-data area at offset 0. Used by the BLE Touch & Go feature to
 * publish an NDEF Text record encoded by bluetooth_manager.
 */
esp_err_t nfc_write_raw_ndef(const uint8_t *data, size_t len)
{
    if (!data || len == 0)
        return ESP_ERR_INVALID_ARG;
    if (len > NFC_USER_DATA_CAPACITY)
        return ESP_ERR_INVALID_SIZE;
    return nfc_write_user_data(0, data, len);
}
