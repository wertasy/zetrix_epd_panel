...
static void render_qr_placeholder(page_renderer_t *self, uint8_t *fb, int width, int height,
                                  int box_x, int box_y, int box_size)
{
    car_move_page_t *r = (car_move_page_t *)self;
    ...
    rawdraw_draw_circle_border(fb, width, height,
                               (rawdraw_point_t){box_x + box_size / 2, box_y + box_size / 2}, dot_r,
                               STYLE_BORDER_MEDIUM, border);
...
static void render_qr_area(page_renderer_t *self, uint8_t *fb, int width, int height,
                           int qr_x, int qr_y, int qr_size)
{
...
PAGE_REGISTER(UI_PAGE_CAR_MOVE, "扫码挪车", NULL, true, 50, &car_move_page_ops,
              &s_car_move_instance.base);
