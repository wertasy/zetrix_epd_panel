...
static const nvs_i32_key_entry_t s_i32_keys[NVS_I32_KEY_COUNT] = {
    {K_AP_XFER_BOOT, NVS_I32_AP_XFER_BOOT}, {K_BT_ENABLED, NVS_I32_BT_ENABLED},
    {K_CAL_DAY, NVS_I32_CAL_DAY},           {K_CAL_LUNAR, NVS_I32_CAL_LUNAR},
    {K_CAL_MONTH, NVS_I32_CAL_MONTH},       {K_CAL_YEAR, NVS_I32_CAL_YEAR},
    {K_EPD_LAST_TS, NVS_I32_EPD_LAST_TS},   {K_EPD_LIFETIME, NVS_I32_EPD_LIFETIME},
    {K_EPD_PARTIAL, NVS_I32_EPD_PARTIAL},   {K_UI_LIFEBAR, NVS_I32_UI_LIFEBAR},
    {K_UI_PAGE, NVS_I32_UI_PAGE},           {K_UI_SCROLL, NVS_I32_UI_SCROLL},
    {K_WEATHER_AUTO, NVS_I32_WEATHER_AUTO}, {K_WEATHER_INTERVAL, NVS_I32_WEATHER_INTERVAL},
};
...
{
    const nvs_i32_key_entry_t *hit =
        (const nvs_i32_key_entry_t *)bsearch(key, s_i32_keys, NVS_I32_KEY_COUNT, sizeof(s_i32_keys[0]), i32_key_cmp);
    return hit ? hit->id : -1;
}
...
static bool map_get_i32(const char *key, int32_t *out)
{
    ...
    case NVS_I32_AP_XFER_BOOT:
        *out = s_cache.ap_transfer_boot ? 1 : 0;
        return true;
    case NVS_I32_BT_ENABLED:
        *out = s_cache.ble.enabled ? 1 : 0;
        return true;
    case NVS_I32_CAL_DAY:
        *out = s_cache.calendar.selected_day;
        return true;
    case NVS_I32_CAL_LUNAR:
        *out = s_cache.calendar.show_lunar ? 1 : 0;
        return true;
    case NVS_I32_CAL_MONTH:
        *out = s_cache.calendar.selected_month;
        return true;
    case NVS_I32_CAL_YEAR:
        *out = s_cache.calendar.selected_year;
        return true;
    case NVS_I32_EPD_LAST_TS:
        *out = s_cache.epd.last_refresh_timestamp;
        return true;
    case NVS_I32_EPD_LIFETIME:
        *out = s_cache.epd.lifetime_refreshes;
        return true;
    case NVS_I32_EPD_PARTIAL:
        *out = s_cache.epd.partial_count;
        return true;
    case NVS_I32_UI_LIFEBAR:
        *out = s_cache.ui.lifebar_visible;
        return true;
    case NVS_I32_UI_PAGE:
        *out = s_cache.ui.last_page;
        return true;
    case NVS_I32_UI_SCROLL:
        *out = s_cache.ui.summary_scroll;
        return true;
    case NVS_I32_WEATHER_AUTO:
        *out = s_cache.weather.auto_update ? 1 : 0;
        return true;
    case NVS_I32_WEATHER_INTERVAL:
        *out = s_cache.weather.update_interval;
        return true;
    default:
        return false;
    }
...
static bool map_set_i32(const char *key, int32_t value)
{
    switch (map_i32_key(key)) {
    case NVS_I32_AP_XFER_BOOT:
        s_cache.ap_transfer_boot = (value != 0);
        break;
    case NVS_I32_BT_ENABLED:
        s_cache.ble.enabled = (value != 0);
        break;
    case NVS_I32_CAL_DAY:
        s_cache.calendar.selected_day = (int)value;
        break;
    case NVS_I32_CAL_LUNAR:
        s_cache.calendar.show_lunar = (value != 0);
        break;
    case NVS_I32_CAL_MONTH:
        s_cache.calendar.selected_month = (int)value;
        break;
    case NVS_I32_CAL_YEAR:
        s_cache.calendar.selected_year = (int)value;
        break;
    case NVS_I32_EPD_LAST_TS:
        s_cache.epd.last_refresh_timestamp = (int)value;
        break;
    case NVS_I32_EPD_LIFETIME:
        s_cache.epd.lifetime_refreshes = (int)value;
        break;
    case NVS_I32_EPD_PARTIAL:
        s_cache.epd.partial_count = (int)value;
        break;
    case NVS_I32_UI_LIFEBAR:
        s_cache.ui.lifebar_visible = (int)value;
        break;
    case NVS_I32_UI_PAGE:
        s_cache.ui.last_page = (int)value;
        break;
    case NVS_I32_UI_SCROLL:
        s_cache.ui.summary_scroll = (int)value;
        break;
    case NVS_I32_WEATHER_AUTO:
        s_cache.weather.auto_update = (value != 0);
        break;
    case NVS_I32_WEATHER_INTERVAL:
        s_cache.weather.update_interval = (int)value;
        break;
    default:
        return false;
    }
