...
static void spi_gpio_init(void) {
    gpio_config_t gpio_conf = {
        ...
        .pin_bit_mask = (1ULL << g_display.lcd_spi_data.rst) | (1ULL << g_display.lcd_spi_data.dc) | (1ULL << g_display.lcd_spi_data.cs),
...
static void update_display_busy_locked(void) {
    const bool busy = g_display.pending || g_display.urgent_refresh || g_display.force_full_refresh || g_display.refresh_in_progress;
...
static void refresh_task_loop(void *arg) {
    ...
    while (true) {
        ...
        while (debounce > 0 && (xTaskGetTickCount() - t0) < debounce) {
            ...
            xSemaphoreTake(g_display.dirty_mutex, portMAX_DELAY);
            if (g_display.pending && g_display.dirty.w > 0 && g_display.dirty.h > 0) {
                int x1 = r.w > 0 ? (r.x < g_display.dirty.x ? r.x : g_display.dirty.x) : g_display.dirty.x;
                int y1 = r.h > 0 ? (r.y < g_display.dirty.y ? r.y : g_display.dirty.y) : g_display.dirty.y;
                int x2 = r.w > 0 ? ((r.x + r.w) > (g_display.dirty.x + g_display.dirty.w) ? (r.x + r.w) : (g_display.dirty.x + g_display.dirty.w)) : (g_display.dirty.x + g_display.dirty.w);
                int y2 = r.h > 0 ? ((r.y + r.h) > (g_display.dirty.y + g_display.dirty.h) ? (r.y + r.h) : (g_display.dirty.y + g_display.dirty.h)) : (g_display.dirty.y + g_display.dirty.h);
                r.x = x1;
                r.y = y1;
                r.w = x2 - x1;
                r.h = y2 - y1;
        ...
        frame_diff_result_t result = analyze_frame_diff(g_display.prev_buffer, g_display.tx_buf, g_display.width, g_display.height);
...
void custom_lcd_display_init(const custom_lcd_spi_t* spi_data) {
    ...
    g_display.buffer = (uint8_t *)heap_caps_malloc(spi_data->buffer_len, MALLOC_CAP_SPIRAM);
    if (!g_display.buffer) {
        g_display.buffer = (uint8_t *)malloc(spi_data->buffer_len);
    }
    assert(g_display.buffer);
    memset(g_display.buffer, 0x55, spi_data->buffer_len);

    g_display.prev_buffer = (uint8_t *)heap_caps_malloc(spi_data->buffer_len, MALLOC_CAP_SPIRAM);
    if (!g_display.prev_buffer) {
        g_display.prev_buffer = (uint8_t *)malloc(spi_data->buffer_len);
    }
    assert(g_display.prev_buffer);
    memset(g_display.prev_buffer, 0x55, spi_data->buffer_len);

    g_display.tx_buf = (uint8_t *)heap_caps_malloc(spi_data->buffer_len, MALLOC_CAP_SPIRAM);
    if (!g_display.tx_buf) {
        g_display.tx_buf = (uint8_t *)malloc(spi_data->buffer_len);
    }
    assert(g_display.tx_buf);
    memset(g_display.tx_buf, 0x55, spi_data->buffer_len);
    ...
    epd_init();
    epd_clear();
    memcpy(g_display.prev_buffer, g_display.buffer, spi_data->buffer_len);
    
    memcpy(g_display.tx_buf, g_display.buffer, spi_data->buffer_len);
    epd_display_full();
