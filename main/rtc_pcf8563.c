...
bool pcf8563_set_time(const struct tm* local_tm) {
    if (!g_board.rtc_device) return false;
    board_i2c_write_reg(g_board.rtc_device, REG_SECONDS, to_bcd(local_tm->tm_sec) & 0x7F);
    board_i2c_write_reg(g_board.rtc_device, REG_MINUTES, to_bcd(local_tm->tm_min) & 0x7F);
    board_i2c_write_reg(g_board.rtc_device, REG_HOURS, to_bcd(local_tm->tm_hour) & 0x3F);
    board_i2c_write_reg(g_board.rtc_device, REG_DAYS, to_bcd(local_tm->tm_mday) & 0x3F);
    board_i2c_write_reg(g_board.rtc_device, REG_WEEKDAYS, to_bcd(local_tm->tm_wday) & 0x07);
    board_i2c_write_reg(g_board.rtc_device, REG_MONTHS, to_bcd(local_tm->tm_mon + 1) & 0x1F);
    board_i2c_write_reg(g_board.rtc_device, REG_YEARS, to_bcd(local_tm->tm_year % 100));
    return true;
}
...
bool pcf8563_set_alarm(const struct tm* target_local_tm) {
    if (!g_board.rtc_device) return false;
    board_i2c_write_reg(g_board.rtc_device, REG_ALARM_MINUTE, to_bcd(target_local_tm->tm_min) & 0x7F);
    board_i2c_write_reg(g_board.rtc_device, REG_ALARM_HOUR, to_bcd(target_local_tm->tm_hour) & 0x3F);
    board_i2c_write_reg(g_board.rtc_device, REG_ALARM_DAY, to_bcd(target_local_tm->tm_mday) & 0x3F);
    board_i2c_write_reg(g_board.rtc_device, REG_ALARM_WEEKDAY, ALARM_DISABLE_BIT);
...
bool pcf8563_disable_alarm(void) {
    if (!g_board.rtc_device) return false;
    board_i2c_write_reg(g_board.rtc_device, REG_ALARM_MINUTE, ALARM_DISABLE_BIT);
    board_i2c_write_reg(g_board.rtc_device, REG_ALARM_HOUR, ALARM_DISABLE_BIT);
    board_i2c_write_reg(g_board.rtc_device, REG_ALARM_DAY, ALARM_DISABLE_BIT);
    board_i2c_write_reg(g_board.rtc_device, REG_ALARM_WEEKDAY, ALARM_DISABLE_BIT);
...
bool pcf8563_clear_alarm_flag(void) {
    if (!g_board.rtc_device) return false;
    uint8_t ctrl2 = board_i2c_read_reg(g_board.rtc_device, REG_CTRL2) & CTRL2_WRITABLE_MASK;
    ctrl2 &= ~CTRL2_ALARM_FLAG;
    board_i2c_write_reg(g_board.rtc_device, REG_CTRL2, ctrl2);
    return true;
}
...
bool pcf8563_start_countdown_timer(uint8_t seconds) {
    ...
    pcf8563_stop_countdown_timer();
    pcf8563_clear_timer_flag();
    board_i2c_write_reg(g_board.rtc_device, REG_TIMER_VALUE, timer_value);
    board_i2c_write_reg(g_board.rtc_device, REG_TIMER_CONTROL, (uint8_t)(TIMER_ENABLE | TIMER_FREQ_1HZ));

    uint8_t ctrl2 = board_i2c_read_reg(g_board.rtc_device, REG_CTRL2) & CTRL2_WRITABLE_MASK;
    ctrl2 |= CTRL2_TIMER_INT_ENABLE;
    board_i2c_write_reg(g_board.rtc_device, REG_CTRL2, ctrl2);
    return true;
}
...
bool pcf8563_stop_countdown_timer(void) {
    if (!g_board.rtc_device) return false;
    board_i2c_write_reg(g_board.rtc_device, REG_TIMER_CONTROL, 0x00);
    uint8_t ctrl2 = board_i2c_read_reg(g_board.rtc_device, REG_CTRL2) & CTRL2_WRITABLE_MASK;
    ctrl2 &= ~CTRL2_TIMER_INT_ENABLE;
    board_i2c_write_reg(g_board.rtc_device, REG_CTRL2, ctrl2);
    return true;
